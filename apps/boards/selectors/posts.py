...
def get_post_queryset_by_board_id(board_id: int) -> QuerySet[Optional[Post]]:
    ...
    return (
        ...
        .filter(board_id=board_id, deleted_at__isnull=True)
        .annotate(_comment_count=Count("comment"))
    )
...
def get_post_by_id_board_id(
    post_id: int, board_id: int
) -> Post:
    """post by id and board id selector"""
